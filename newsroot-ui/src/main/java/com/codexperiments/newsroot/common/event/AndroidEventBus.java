package com.codexperiments.newsroot.common.event;
...
/**
 * Classe permettant la transmission d'�v�nement inter-composants (ex: entre vue graphique d'une application). L'EventManager
 * permet donc de notifier des composants tiers en �vitant un couplage fort.
 * 
 * Les EventManager peuvent �tre cha�n�s entre eux: des composants peuvent par exemple s'enregistrer sur un "canal" local via un
 * EventManager commun et utiliser un EventManager global pour communiquer avec le reste de l'application. Sur RPA, le principe
 * utilis� est le suivant: chaque vue (au sens MVP/MVC) parent et ses enfants directs sont connect�s sur un "canal local". Ce
 * principe s'appliquant r�cursivement de fa�on hi�rarchique. Le parent peux forwarder les �v�nements globaux aux enfant et
 * r�ciproquement les �v�nements locaux vers l'ext�rieur.
 * 
 * L'objectif est que, comme avec le MVC-Hi�rarchique, le parent ait la r�sponsabilit� du transfert des �v�nements et que ceux-ci
 * soit transmis de fa�on ordonn� (afin d'�viter les interconnexions multiples de gr� � gr� comme on peut l'obtenir avec le
 * pattern Observable standard). De plus cela permet de s�parer facilement plusieurs instances d'une IHM: par exemple si plusieurs
 * composants communiquent au sein d'un m�me �cran mais que plusieurs instances de l'�cran (onglets, etc.) peuvent exister. Il n'y
 * a alors pas de "parasitage" ou de n�cessiter de tester la source de l'�v�nement.
 * 
 * Note: Les �v�nements sont forward�s � d'autres EventManagers syst�matiquement "apr�s" avoir notifi� les listeners.
 */
public class AndroidEventBus implements EventBus
{
    ...
    {
        Iterator<Class<?>> lClassIterator = mEventMap.keySet().iterator();
        while (lClassIterator.hasNext()) {
            Class<?> event = lClassIterator.next();
            Collection<Object> listeners = mEventMap.get(event);
            if (listeners.size() == 1 && listeners.contains(listener)) {
                lClassIterator.remove();
            } else {
    ...
    /**
     * Renvoie l'ensemble des interfaces implémentées par une classe, y compris les classes parentes (pas les interfaces parentes
     * des interfaces attention).
     * 
