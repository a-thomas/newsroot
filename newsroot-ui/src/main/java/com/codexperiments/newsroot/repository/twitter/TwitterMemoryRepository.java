...
public class TwitterMemoryRepository implements TwitterRepository {
    ...
    private static class CacheEntry {
        ...
        private Observable<TweetPage> cacheTweetPages(final Observable<TweetPage> pTweetPages) {
            return Observable.create(new OnSubscribeFunc<TweetPage>() {
                public Subscription onSubscribe(final Observer<? super TweetPage> pObserver) {
                    return pTweetPages.subscribe(new Observer<TweetPage>() {
                        public void onNext(TweetPage pTweetPage) {
                            mEmpty = false;

                            TimeGap lTimeGap = pTweetPage.timeGap();
                            ...
                            if (lTimeGap.isInnerGap()) {
                                ...
                                if (lGapIndex != -1 && lTimeGap.equals(mTweets.get(lGapIndex))) {
                                    ...
                                    mTweets.addAll(lGapIndex + 1, pTweetPage.tweets());
                                }
                            }
                            // Past or future gap.
                            else {
                                ...
                                if (lTimeGap.isFutureGap()) {
                                    ...
                                    mTweets.addAll(pTweetPage.tweets());
                                } else if (lTimeGap.isPastGap()) {
                                    mTweets.addAll(0, pTweetPage.tweets());
                                }
                            }
